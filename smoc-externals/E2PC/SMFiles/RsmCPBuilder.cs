...
{
    public class RsmCPBuilder: IRSMBuilder
    {
        private string _name;
        private bool _logging;
        public RsmCPBuilder(string name, bool logging)
        {
            _name = name;
            _logging = logging;
        }
        public StateMachine<State, Event> BuildStateMachine(string smName)
        {
            ...
            sm.Configure(State.STATE7).Permit(Event.TRIVIAL6, State.STATE8);
            sm.Configure(State.STATE8).Permit(Event.TRIVIAL7, State.STATE9);
            sm.Configure(State.STATE9).Permit(Event.TRIVIAL8, State.STATE10);
            sm.Configure(State.STATE10).Permit(Event.TRIVIAL9, State.STATE11);
            sm.Configure(State.STATE11).Permit(Event.TRIVIAL10, State.STATE12);
            sm.Configure(State.STATE12).Permit(Event.TRIVIAL11, State.STATE13);
            sm.Configure(State.STATE13).Permit(Event.TRIVIAL12, State.STATE14);
            sm.Configure(State.STATE14).Permit(Event.TRIVIAL13, State.STATE15);
            sm.Configure(State.STATE15).Permit(Event.WRITE, State.STATE16);
            sm.Configure(State.STATE16).Permit(Event.TRIVIAL14, State.STATE17);
